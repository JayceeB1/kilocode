#!/usr/bin/env python3
"""Convenience wrapper to refresh LLM context snapshots for supervisor packages."""
...
DEFAULT_DIRECTORIES: Sequence[Path] = (
    Path("packages/supervisor-service"),
    Path("packages/supervisor-sidecar"),
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Rebuild the llm-context snapshots used to prime the supervisor LLM."
        )
    )
    parser.add_argument(
        "directories",
        nargs="*",
        type=Path,
        help="Directories (relative to --root) to summarize. Defaults to supervisor packages.",
    )
    parser.add_argument(
        "--root",
        type=Path,
